...
export default function ConnectionsSection({
  ...
  const [togglingNotifications, setTogglingNotifications] = useState<
    number | null
  >(null);
  ...
  const handleToggleArticleNotifications = async (
    authorSlug: string,
    followId: number,
  ) => {
    try {
      setTogglingNotifications(followId);
      const follow = following.find((f) => f.id === followId);
      if (!follow) return;

      const newNotificationStatus = !follow.article_notifications;
      const token = localStorage.getItem("token");

      const response = await fetch(
        `${API_BASE_URL}/authors/${authorSlug}/follow-notifications/`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Token ${token}`,
          ...
          body: JSON.stringify({
            article_notifications: newNotificationStatus,
          }),
        },
      );
      ...
      if (response.ok) {
        setFollowing(
          following.map((f) =>
            f.id === followId
              ? { ...f, article_notifications: newNotificationStatus }
              : f,
          ),
        );
      }
    } catch (err) {
      console.error("Error toggling article notifications:", err);
  ...
  const handleUnfollow = async (followingSlug: string, followId: number) => {
    if (!confirm("Are you sure you want to unfollow this author?")) {
      return;
    }
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${API_BASE_URL}/authors/${followingSlug}/unfollow/`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Token ${token}`,
    ...
    >
      ...
      <div className="px-4 md:px-8 py-4 md:py-6 border-b border-slate-200/50 dark:border-gray-700 bg-gradient-to-r from-white to-slate-50/50 dark:from-gray-800 dark:to-gray-700/50">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 md:gap-4 mb-4">
          <div>
            <h2 className="text-xl md:text-3xl font-bold bg-gradient-to-br from-blue-600 to-purple-600 dark:from-blue-300 dark:to-purple-300 bg-clip-text text-transparent mb-1 md:mb-2">
              Your Connections
            </h2>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
                          ...
                          {/* Actions for Following Section Only */}
                          <div className="flex items-center gap-2">
                            {/* Article Notification Toggle */}
                            {/* Article Notification Toggle */}
                            <button
                              onClick={() =>
                                handleToggleArticleNotifications(
                                  followingItem.following_slug,
                                  followingItem.id,
                                )
                              }
                              disabled={
                                togglingNotifications === followingItem.id
                              }
                              className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-lg transition-all duration-300 font-medium text-sm shadow-sm hover:shadow-md ${
                                followingItem.article_notifications
                                  ? "bg-gradient-to-r from-amber-500 to-yellow-600 text-white hover:from-amber-600 hover:to-yellow-700"
                                  : "bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600"
                              }`}
                              title={
                                followingItem.article_notifications
                                  ? "Disable article notifications"
                            ...
                            <button
                              ...
                              disabled={unfollowingId === followingItem.id}
                              className="inline-flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-red-500 to-rose-600 text-white rounded-lg font-medium text-sm shadow-sm hover:shadow-md hover:from-red-600 hover:to-rose-700 transition-all duration-300 disabled:opacity-50"
                              title={`Unfollow ${followingItem.following_name}`}
                            >
                              {unfollowingId === followingItem.id ? (
